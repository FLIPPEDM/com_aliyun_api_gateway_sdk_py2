import httplib
import urllib
from com_aliyun_api_gateway_sdk_py2.common import constant
from com_aliyun_api_gateway_sdk_py2.http.request import Request


class Response(Request):
    def __init__(self, host=None, url=None, method=constant.GET, headers={}, protocol=constant.HTTP, content_type=None, content=None, port=None, key_file=None, cert_file=None, time_out=None):
    ...
    def get_http_response(self):
        ...
        try:
            ...
            self.__connection.request(method=self.get_method(), url=self.get_url(), body=post_data, headers=self.get_headers())
    ...
    def get_http_response_object(self):
        ...
        try:
            ...
            self.__connection.connect()
            self.__connection.request(method=self.get_method(), url=self.get_url(), body=self.get_body(), headers=self.get_headers())
            response = self.__connection.getresponse()
            return response.status, response.getheaders(), response.read()
        except Exception as e:
            return None, None, None
    ...
    def get_https_response(self):
        try:
            self.__port = 443
            self.__connection = httplib.HTTPSConnection(self.parse_host(), self.__port, cert_file=self.__cert_file, key_file=self.__key_file)
            self.__connection.connect()
            post_data = None
            if self.get_content_type() == constant.CONTENT_TYPE_FORM and self.get_body():
                post_data = urllib.urlencode(self.get_body())
    ...
    def get_https_response_object(self):
        ...
        try:
            self.__port = 443
            self.__connection = httplib.HTTPSConnection(self.get_host(), self.__port, cert_file=self.__cert_file, key_file=self.__key_file)
            self.__connection.connect()
            self.__connection.request(method=self.get_method(), url=self.get_url(), body=self.get_body(), headers=self.get_headers())
            response = self.__connection.getresponse()
            return response.status, response.getheaders(), response.read()
